...
pub fn config_to_toml(config: &AppConfig) -> Result<String> {
    config_to_commented_toml(config)
}

/// Serialize the config to TOML with section header comments, plus commented
/// defaults for unconfigured optional sections (webhooks, upstream_proxy).
pub fn config_to_commented_toml(config: &AppConfig) -> Result<String> {
    // Serialize the full config as valid TOML using the crate's serializer
    let base = toml::to_string_pretty(config)
        .map_err(|e| anyhow::anyhow!("Failed to serialize config: {e}"))?;

    let sep = "# =============================================================================";
    let section_comments: &[(&str, &str)] = &[
        ("[server]", "# [server] — SSH and SOCKS5 server settings"),
        ("[shell]", "# [shell] — Virtual shell settings"),
        ("[limits]", "# [limits] — Connection and rate limits"),
        (
            "[security]",
            "# [security] — Authentication and security settings",
        ),
        ("[logging]", "# [logging] — Logging configuration"),
        ("[metrics]", "# [metrics] — Prometheus metrics server"),
        ("[api]", "# [api] — Management REST API"),
        ("[motd]", "# [motd] — Message of the Day (template-based)"),
        ("[geoip]", "# [geoip] — GeoIP lookup (MaxMind DB)"),
        ("[acl]", "# [acl] — Global Access Control Lists"),
        ...
        (
            "[upstream_proxy]",
            "# [upstream_proxy] — Global upstream SOCKS5 proxy",
        ),
        (
            "[[webhooks]]",
            "# [[webhooks]] — HTTP webhook notifications",
        ),
        ("[[users]]", "# [[users]] — User accounts"),
        (
            "[[groups]]",
            "# [[groups]] — User groups with shared settings",
        ),
        (
            "[[maintenance_windows]]",
    ...
    out.push_str("# sks5 configuration \u{2014} generated by `sks5 wizard`\n");
    out.push_str("# Documentation: https://github.com/galti3r/sks5\n\n");

    let mut seen = std::collections::HashSet::new();
    for line in base.lines() {
        let trimmed = line.trim();

        // Inject comment banner before top-level section headers (no dots)
        if trimmed.starts_with('[') && !trimmed.contains('.') {
            for &(section, comment) in section_comments {
                if trimmed == section && seen.insert(section) {
                    out.push_str(sep);
                    out.push('\n');
    ...
    // Commented placeholders for unconfigured optional sections
    if config.webhooks.is_empty() {
        out.push('\n');
        out.push_str(sep);
        out.push('\n');
        out.push_str("# [[webhooks]] \u{2014} HTTP webhook notifications (none configured)\n");
        out.push_str(sep);
        out.push('\n');
        out.push_str("# [[webhooks]]\n");
        out.push_str("# url = \"https://hooks.example.com/sks5\"\n");
        out.push_str("# format = \"generic\"  # generic, slack, discord, or custom\n");
        out.push_str("# events = [\"auth_failure\", \"ban\"]\n");
        out.push_str("# secret = \"hmac-secret\"\n");
    }
    ...
    if config.upstream_proxy.is_none() {
        ...
        out.push_str(sep);
        out.push('\n');
        out.push_str("# [upstream_proxy] \u{2014} Global upstream SOCKS5 proxy (not configured)\n");
        out.push_str(sep);
        out.push('\n');
        out.push_str("# [upstream_proxy]\n");
        out.push_str("# url = \"socks5://proxy.example.com:1080\"\n");
    }
